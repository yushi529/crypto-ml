...
このファイルは、AI コーディングエージェントがこのリポジトリで作業する際のガイダンスです。

## プロジェクト概要

`crypto-ml` — 暗号資産（クリプト）の機械学習プロジェクト。
現在は OHLCV データ取得、時系列可視化、単純移動平均クロス戦略のバックテストを実装済みです。実際のコード・依存関係・ディレクトリ構成が変わったら、本ファイルも更新してください。
...
- 市場データの可視化:
  ```bash
  uv run python -m src.visualization.plot_ohlcv \
      --input data/btc_usdt_1h.csv \
      --output reports/figures/btc_usdt_1h_overview.png
  ```
- SMA クロス戦略のバックテスト:
  ```bash
  uv run python -m src.backtest.sma_crossover \
      --input data/btc_usdt_1h.csv \
      --output-dir reports/backtest/sma_24_168 \
      --fast-window 24 --slow-window 168 --exclude-last
  ```
- テスト: `uv run python -m unittest discover -v`
...
- `src/visualization/plot_ohlcv.py` — OHLCV CSV の終値・出来高・1時間リターンを3段の時系列グラフとして保存する CLI。
- `src/backtest/sma_crossover.py` — ロング／キャッシュ型 SMA クロス戦略を、1本遅延シグナルと取引コスト込みで検証する CLI。リターン・リスク・取引統計、資産曲線、Markdown レポートを出力する。
- `reports/` — 可視化画像とバックテストレポートの保存先。
- `tests/` — 標準ライブラリ `unittest` を使ったテスト。
